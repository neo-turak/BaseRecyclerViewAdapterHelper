...
//        mReadOnlyList = Collections.unmodifiableList(adapterData);
    ...
    fun remove(t: T) {
        val previousList: List<T> = adapter.items
        val index = adapter.items.indexOf(t)
        if (index == -1) return
        adapter.items.removeAt(index)
...
//        mReadOnlyList = Collections.unmodifiableList(adapterData);
    ...
    fun submitList(newList: MutableList<T>?, commitCallback: Runnable? = null) {
        ...
        if (newList === adapter.items) {
            // nothing to do (Note - still had to inc generation, since may have ongoing work)
            commitCallback?.run()
            return
        }
        val oldList: List<T> = adapter.items
        // fast simple remove all
        if (newList == null) {
            val countRemoved: Int = adapter.items.size
    ...
    private fun onCurrentListChanged(previousList: List<T>,
        ...
        for (listener in mListeners) {
            listener.onCurrentListChanged(previousList, adapter.items)
        }
        commitCallback?.run()
    }
