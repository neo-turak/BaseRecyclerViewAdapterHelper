...
constructor(@LayoutRes private val layoutResId: Int,
    ...
    /**
     * items, Only allowed to get.
     * 数据, 只允许 get。
     */
    var items: MutableList<T> = mItems ?: mutableListOf()
        internal set
     ...
     */
    protected open fun convert(holder: VH, item: T, payloads: MutableList<Any>) {}
    ...
    val footerViewPosition: Int
        get() {
            if (hasEmptyView()) {
                ...
                return headerLayoutCount + items.size
            }
            return -1
        }
     ...
     * it doesn't change the [BaseQuickAdapter.items] reference
     ...
     * @param position
     */
    open fun addData(@IntRange(from = 0) position: Int, t: T) {
    ...
    open fun addData(item: T) {
        this.items.add(item)
        notifyItemInserted(this.items.size + headerLayoutCount)
        compatibilityDataSizeChanged(1)
    }
